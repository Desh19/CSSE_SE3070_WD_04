...
export const insertTravel = async (request, response, next) => {
	...
	const { error } =validateTravel(request.body);
		if (error){
			response.status(500).send({ message: error.details[0].message });
			next();
		}else{
...
// Insert one user
export const setDestination = async (request, response, next) => {

			
	await TravelService.setDestination(request.body)
		.then((data) => {
			// request.handleResponse.successRespond(response)(data);
			response.status(200).send({status:"Success",data:data})
			next();
